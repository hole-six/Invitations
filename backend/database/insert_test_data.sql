-- ═══════════════════════════════════════════════════════════════
-- INSERT TEST DATA - Wedding Invitation System
-- ═══════════════════════════════════════════════════════════════
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        @keyframes floatCloud {
            ...
            10% { opacity: 0.8; }
            90% { opacity: 0.8; }
            100% { transform: translate(400px, 20px); opacity: 0; }
        }
        .cloud {
            position: absolute;
            top: 150px;
...
<body>
    <div class="container">
        ...
        <img src="https://cdn-icons-png.flaticon.com/512/414/414927.png" alt="cloud" class="cloud">
    </div>
</body>
</html>',
    NULL,
...
SELECT '✅ Template inserted!' as status;

-- ============================================
-- 2. CREATE TEST INVITATION
-- ============================================

SET @template_id = (SELECT id FROM templates WHERE slug = 'passport-to-love-html' LIMIT 1);
...
SELECT 
    ...
    1,
    @template_id,
    'Passport to Love - Test Invitation',
    'passport-to-love-test',
    'html',
    html_content,
    NULL,
    'published',
...
SELECT '═══════════════════════════════════════════' as '';
SELECT '✅ DỮ LIỆU TEST ĐÃ ĐƯỢC NHẬP!' as '';
SELECT '═══════════════════════════════════════════' as '';

SELECT '' as '';
SELECT 'TEMPLATE:' as '';
SELECT id, name, slug FROM templates WHERE slug = 'passport-to-love-html';

SELECT '' as '';
SELECT 'INVITATION:' as '';
SELECT id, title, slug, status, visibility, template_type, groom_name, bride_name 
FROM invitations 
WHERE slug = 'passport-to-love-test';

SELECT '' as '';
SELECT '═══════════════════════════════════════════' as '';
SELECT 'XEM THIỆP MỜI TẠI:' as '';
SELECT 'http://localhost:5173/invitation/passport-to-love-test' as 'URL';
SELECT '═══════════════════════════════════════════' as '';
